...
describe('Cell', () => {
  ...
  it('renders a looping muted demo video instead of the image when present', () => {
    render(<Cell data={{ ...mockProject, video: '/videos/demo.mp4' }} />);
    const video = document.querySelector('.project-card-image video');
    expect(video).toBeInTheDocument();
    expect(video).toHaveAttribute('src', expect.stringContaining('demo.mp4'));
    expect(video).toHaveAttribute('poster', mockProject.image);
    expect(video).toHaveAttribute('loop');
    // React sets `muted` as a DOM property rather than an HTML attribute.
    expect((video as HTMLVideoElement).muted).toBe(true);
    expect(
      document.querySelector('.project-card-image img'),
