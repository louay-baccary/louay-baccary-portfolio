...
- **Books page**: Static data in `src/data/books.ts` (title, author, image, optional link, status: 'reading' | 'read'). Cover images live in `public/images/books/`. No markdown pipeline, no drafts
- **Design system**: Two type roles, and every element should pick one deliberately: `--font-display`/`--font-body` (Inter) for names, headings, and prose, and `--font-mono` (JetBrains Mono) for data, dates, labels, and buttons. Structure is carried by hairlines (`--rule`) and near-square radii, not by shadow and float. A heavy `--color-fg-bold` rule opens a section; a `--color-border` hairline divides within one
...
- **Signal colour discipline**: `--color-accent` (amber) is the single accent color, used for structure, links, and values that are live or in progress alike — the ticking age, a role with no end date. Using it decoratively elsewhere is what makes the rest of it stop meaning anything. `--color-signal` is the text-safe value; use `--color-signal-mark` for non-text marks only
- **Link accents and button fills are different tokens**: `--color-accent` is tuned for text on the page background and is too light in dark mode to sit behind white text. Filled controls use `--color-accent-fill` / `--color-accent-fill-hover` with `--color-on-accent`
